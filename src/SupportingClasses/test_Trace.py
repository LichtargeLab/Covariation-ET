...
from Trace import (Trace, init_characterization_pool, characterization, init_define_disjoint_sets_pool,
                   define_disjoint_sets, init_trace_groups, trace_groups, init_trace_ranks, trace_ranks,
                   load_freq_table, load_numpy_array)
...
class TestTrace(TestBase):
    ...
    def test1a_init(self):
        ...
        self.assertIsNone(trace_small.disjoint_sets)
    ...
    def test1b_init(self):
        ...
        self.assertIsNone(trace_large.disjoint_sets)
    ...
    def evaluate_characterize_rank_groups(self, aln, phylo_tree, assign, single, pair, processors, low_mem, write_aln,
        ...
        if define_sets:
            positions = np.triu_indices(aln.seq_length, k=1)
            self.assertEqual(len(trace.disjoint_sets), len(positions[0]))
            pair_alphabet = MultiPositionAlphabet(alphabet=Gapped(aln.alphabet), size=2)
            for djs in trace.disjoint_sets:
                cumulative_chars = set()
                cumulative_count = 0
                starting_chars = set()
                ending_chars = set()
                for dj in djs:
                    cumulative_chars |= set(dj)
                    cumulative_count += len(dj)
                    curr_starts = set([x[0] for x in dj])
                    curr_ends = set([x[1] for x in dj])
                    self.assertEqual(len(starting_chars & curr_starts), 0)
                    self.assertEqual(len(ending_chars & curr_ends), 0)
                    starting_chars |= curr_starts
                    ending_chars |= curr_ends
                self.assertEqual(cumulative_chars, set(pair_alphabet.letters))
                self.assertEqual(cumulative_count, len(pair_alphabet.letters))
        else:
            self.assertIsNone(trace.disjoint_sets)
        rmtree(unique_dir)
    ...
    #                                            write_freq_table=True, define_sets=False)
    #
    ...
    #                                            write_freq_table=False, define_sets=False)
    ...
    def test2g_characterize_rank_groups(self):
        ...
        self.evaluate_characterize_rank_groups(aln=self.query_aln_fa_small, phylo_tree=self.phylo_tree_small,
                                               ...
                                               write_freq_table=False, define_sets=True)
    ...
    #         out_dir=self.out_large_dir, low_mem=True, write_sub_aln=False, write_freq_table=False)
    #
    # def evaluate_define_disjoint_sets_pool(self, aln):
    #     pair_alphabet = MultiPositionAlphabet(alphabet=Gapped(aln.alphabet), size=2)
    #     pair_size, _, pair_mapping, pair_reverse = build_mapping(alphabet=pair_alphabet)
    #     indices = np.triu_indices(aln.seq_length, k=1)
    #     init_define_disjoint_sets_pool(aln, pair_size, gap_characters, pair_mapping)
    #     for x in range(len(indices[0])):
    #         i = indices[0][x]
    #         j = indices[0][x]
    #         pos, djs = define_disjoint_sets(x, i, j)
    #         self.assertEqual(pos, x)
    #         self.assertLessEqual(len(djs), pair_size)
    #         unique_count = 0
    #         unique_observations = set()
    #         starts = set()
    #         ends = set()
    #         for s in djs:
    #             curr_starts = set([pair_reverse[x][0] for x in s])
    #             curr_ends = set([pair_reverse[x][1] for x in s])
    #             self.assertEqual(len(starts & curr_starts), 0)
    #             self.assertEqual(len(ends & curr_ends), 0)
    #             self.assertEqual(len(s), len(set(s)))
    #             unique_count += len(s)
    #             unique_observations |= set(s)
    #         self.assertEqual(unique_count, pair_size)
    #         self.assertEqual(unique_observations, set([pair_mapping[x] for x in pair_alphabet.letters]))
    #
    # def test10a_define_disjoint_sets_pool(self):
    #     # Test pool initialization function and mappable function (minimal example) for characterization, small aln
    #     self.evaluate_define_disjoint_sets_pool(aln=self.query_aln_fa_small)
    #
    # def test10b_define_disjoint_sets_pool(self):
    #     # Test pool initialization function and mappable function (minimal example) for characterization, large aln
    #     self.evaluate_define_disjoint_sets_pool(aln=self.query_aln_fa_large)
    ...
    #         out_dir=self.out_large_dir, write_out_aln=False, write_out_freq_table=False)
    #
    # def test15e_trace_pool_functions(self):
    #     # Test the pool functions outside of a multiprocessing environment for the large alignment, paired positions,
    #     # and average product corrected mutual information (MIp) metric (all ranks)
    #     self.evaluate_trace_pool_functions_identity_metric(
    #         aln=self.query_aln_fa_small, phylo_tree=self.phylo_tree_small, assign=self.assignments_small, single=False,
    ...
    #         out_dir=self.out_small_dir, write_out_aln=False, write_out_freq_table=False)
    #
    # def test15f_trace_pool_functions(self):
    #     # Test the pool functions outside of a multiprocessing environment for the large alignment, paired positions,
    #     # and average product corrected mutual information (MIp) metric (all ranks)
    #     self.evaluate_trace_pool_functions_identity_metric(
    #         aln=self.query_aln_fa_small, phylo_tree=self.phylo_tree_small, assign=self.assignments_custom_small,
    ...
    #         out_dir=self.out_small_dir, write_out_aln=False, write_out_freq_table=False)
    #
    # def test15g_trace_pool_functions(self):
    #     # Test the pool functions outside of a multiprocessing environment for the large alignment, paired positions,
    #     # and average product corrected mutual information (MIp) metric (all ranks)
    #     self.evaluate_trace_pool_functions_identity_metric(
    #         aln=self.query_aln_fa_large, phylo_tree=self.phylo_tree_large, assign=self.assignments_large, single=False,
    ...
    #         out_dir=self.out_large_dir, write_out_aln=False, write_out_freq_table=False)
    #
    # def test15h_trace_pool_functions(self):
    #     # Test the pool functions outside of a multiprocessing environment for the large alignment, paired positions,
    #     # and average product corrected mutual information (MIp) metric (custom ranks)
    #     self.evaluate_trace_pool_functions_identity_metric(
    #         aln=self.query_aln_fa_large, phylo_tree=self.phylo_tree_large, assign=self.assignments_custom_large,
