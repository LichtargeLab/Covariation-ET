...
class TestContactScorer(TestBase):
    ...
    def test_19d_score_clustering_of_contact_predictions(self):
        ...
        print(sorted(self.scorer2.query_pdb_mapping.keys()))
        # print(sorted(set(zscore_df_2u.loc[indices, 'Res_i']) | set(zscore_df_2u.loc[indices, 'Res_j'])))
        zscore_df_2ue = self.all_z_scores(mapping=self.scorer2.query_pdb_mapping, special_mapping=final_map, A=A,
                                          L=len(self.scorer2.query_structure.seq[self.scorer2.best_chain]), bias=0,
        ...
        residue_count_diff = np.abs(np.array(zscore_df_2u['Num_Residues']) -
                                    np.array(zscore_df_2ue['Num_Residues']))
        residue_count_passing = residue_count_diff > 1E-16
        print(sorted(A.keys()))
        print(np.array(zscore_df_2u['Res_i'])[residue_count_passing])
        print(np.array(zscore_df_2u['Res_j'])[residue_count_passing])
        print(np.array(zscore_df_2u['Covariance_Score'])[residue_count_passing])
        print(np.array(zscore_df_2u['Num_Residues'])[residue_count_passing])
        print(np.array(zscore_df_2ue['Num_Residues'])[residue_count_passing])
        test = set()
        expected_num = []
        for ind in zscore_df_2u.index:
            if zscore_df_2u.loc[ind, 'Z-Score'] in [None, '-', 'NA']:
                expected_num.append(None)
            else:
                test.add(zscore_df_2u.loc[ind, 'Res_i'])
                test.add(zscore_df_2u.loc[ind, 'Res_j'])
                expected_num.append(len(test))
        expected_num = np.array(expected_num)
        print(expected_num[residue_count_passing])
        print(residue_count_diff[residue_count_passing])
        print(np.min(residue_count_diff[residue_count_passing]))
        print(np.max(residue_count_diff[residue_count_passing]))
        print(np.array(range(len(zscore_df_2u['Res_i'])))[residue_count_passing])
        self.assertEqual(np.sum(residue_count_passing), 0)
        # Res I comparison
        res_i_diff = (np.array(zscore_df_2u['Res_i']) - np.array(zscore_df_2ue['Res_i']))
        res_i_diff = res_i_diff > 1E-16
        self.assertEqual(np.sum(res_i_diff), 0)
        # Res J comparison
        print(zscore_df_2u.columns)
        print(zscore_df_2ue.columns)
        res_j_diff = (np.array(zscore_df_2u['Res_j']) - np.array(zscore_df_2ue['Res_j']))
        res_j_diff = res_j_diff > 1E-16
        self.assertEqual(np.sum(res_j_diff), 0)
        # W Comparison
        w = np.array(zscore_df_2u['W'].replace([None, '-', 'NA'], np.nan))
        expected_w = np.array(zscore_df_2ue['W'].replace([None, '-', 'NA'], np.nan))
        w_nan_ind = np.isnan(w)
        expected_w_nan_ind = np.isnan(expected_w)
        print('HERE')
        print(np.min(np.array(zscore_df_2u['Res_i'])))
        print(np.min(np.array(zscore_df_2u['Res_j'])))
        print(np.max(np.array(zscore_df_2u['Res_i'])))
        print(np.max(np.array(zscore_df_2u['Res_j'])))
        print(w.shape)
        print(w[w_nan_ind])
        print(w[w_nan_ind].shape)
        print(expected_w.shape)
        print(expected_w[expected_w_nan_ind])
        print(expected_w[expected_w_nan_ind].shape)
        disagree = np.logical_xor(w_nan_ind, expected_w_nan_ind)
        print(np.array(zscore_df_2u['Res_i'])[disagree])
        print(np.array(zscore_df_2u['Res_j'])[disagree])
        print(sorted(final_map.keys()))
        print(sorted(self.scorer2.query_pdb_mapping.keys()))
        print(sorted(A.keys()))
        self.assertEqual(w_nan_ind, expected_w_nan_ind, (w[w_nan_ind], expected_w[expected_w_nan_ind]))
        w_diff = np.abs(w - expected_w)
        w_passing = w_diff > 1e-16
        w_passing[w_nan_ind] = 0
        self.assertEqual(np.sum(w_passing), 0, w_diff)
        # W_Ave Comparison
