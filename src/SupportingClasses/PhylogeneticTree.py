...
class PhylogeneticTree(object):
    ...
    def _upgma_tree(self):
        ...
        This actually generates a WPGMA tree, not UPGMA, the ET tree is UPGMA. See Wiki page for examples.
        (https://en.wikipedia.org/wiki/WPGMA)(https://en.wikipedia.org/wiki/UPGMA).
        ...
        def height_of(clade):
            ...
            if clade.is_terminal():
                ...
                height = max(height_of(c) + c.branch_length for c in clade.clades)
            return height
    ...
    def _et_tree(self):
        ...
        This method reproduces the tree used by the Evolutionary Trace method in the past.This is the UPGMA tree 
        construction method which joins nodes, not based on their minimum distance to each other but on the minimum
        average distance between their terminal descendants. (https://en.wikipedia.org/wiki/UPGMA)
        
        
        Returns:
            Bio.Phylo.Newick.Tree: Tree structure generated by the ET method from the distance matrix computed for the
            target alignment.
        """
        ...
        while dm.shape[0] > 1:
            ...
            # Retrieve nodes to merge and construct new inner node
            clade1 = clades[min_i]
            clade2 = clades[min_j]
            inner_clade = BaseTree.Clade(None, inner_name)
            inner_clade.clades.append(clade1)
            inner_clade.clades.append(clade2)
            # Assign branch length to each merged node
            if clade1.is_terminal():
                clade1.branch_length = min_dist * 1.0 / 2
            else:
                clade1.branch_length = min_dist * 1.0 / 2 - height_of(clade1)
    ...
    def construct_tree(self, dm):
        ...
        __distance_matrix and tree attributes of the PhylogeneticTree instance, acting as a switch statement to call the
        correct tree construction method.
    ...
    def annotate_incomplete_tree(self, annotations):
        ...
        Return:
            dict: A dictionary including the annotations from the dictionary provided as input as well as the
            annotations generated by this method, i.e. mapping between the 'Leaf (Sequence ID)' and the 'Annotation(s)'
            columns of the dataframe described below.
        ...
        for node in terminals:
            if node.name in unannotated_terminals:
                root_to_leaf_path = [self.tree.root] + self.tree.get_path(node)[:-1]
