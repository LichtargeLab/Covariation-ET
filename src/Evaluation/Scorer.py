...
class Scorer(object):
    ...
    def plot_auc(self, auc_data, title=None, file_name=None, output_dir=None):
        ...
        if file_name is None:
            file_name = f'{self.query_pdb_mapper.query}_auroc.png'
        ...
        if title is None:
            title = f'Ability to predict positive contacts in {self.query_pdb_mapper.query}'
        plt.title(title)
        plt.legend(loc="lower right")
        plt.savefig(file_name, format='png', dpi=500, fontsize=8)
        plt.close()
    ...
    def plot_auprc(self, auprc_data, title=None, file_name=None, output_dir=None):
        ...
        if (auprc_data[0] is None) and (auprc_data[1] is None) and (auprc_data[2] in {None, '-', 'NA'}):
            return
        if file_name is None:
            file_name = f'{self.query_pdb_mapper.query}_auprc.png'
        if not file_name.endswith('.png'):
            file_name = file_name + '.png'
        if output_dir:
            file_name = os.path.join(output_dir, file_name)
