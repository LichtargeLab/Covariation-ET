...
def Generate_Pair_Mapping(msa_num, ppi=False):
    ...
    #n_comparisons = len([pair_mapping[x][y] for y in pair_mapping[x] for x in pair_mapping])

    return pair_mapping#, n_comparisons
...
def init_CovET(start, end, groupsize, processed_tree, fixed_penalty):
    ...
    #https://stackoverflow.com/questions/22530392/python-multiprocessing-with-shared-library-and-memory-persistance
    #https://stackoverflow.com/questions/6440474/python-multiprocessing



def calculate_paired_scores(xpp):


    c_return_scores = mylib.TraceGroupC_vec(pool_start, pool_end, pool_groupsize, pool_nseqs, xpp)
    #print(c_return_scores)

    mylib.free_mem(c_return_scores)
    return c_return_scores
...
def worker(q):
    while q.qsize() >0 :
        ...
        c_return_scores = calculate_paired_scores(x)
...
def CovET(processed_tree, cores, msa_num, position_matrix, pair_mapping, fixed_penalty):
    ...
    init_CovET(start, end, groupsize, processed_tree, fixed_penalty)
    ...
    print('5')
#
# def CovET(processed_tree, processes, msa_num, position_matrix, pair_mapping, fixed_penalty, n_comp):
#     """
...
#
#     result_list = []
#     result_pbar = tqdm(total=n_comp, unit='variation')
#
...
#     for pos_i in pair_mapping:
#         for pos_j in pair_mapping[pos_i].keys():
#             print(f'Iteration {pos_i}, {pos_j}')
#             pool1.apply_async(calculate_paired_scores, (position_matrix, pos_i, pos_j,), callback=get_result)
#
...
if __name__ == "__main__":
    ...
    print(f'Using {cores} Processors to run CovET')

    results_dictionary = CovET(processed_tree, cores, msa_num, position_matrix, pair_mapping, fixed_penalty)
