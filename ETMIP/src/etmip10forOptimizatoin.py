...
def distance_matrix(alignment_dict):
    ...
    for key in alignment_dict:  # {seqid} = sequence
        for key2 in alignment_dict:
            sumDist = 0.0
            if key > key2:
                for idc, _char in enumerate(alignment_dict[key]):
...
if __name__ == '__main__':
    ...
    currS = time.time()
    wholeMIP_Matrix = whole_analysis(fixed_alignment_dict, aa_list)
    currE = time.time()
    ...
    for clus in ls:
        ...
        for c in clusterset:
            ...
            clusteredMIP_matrix = whole_analysis(new_alignment, aa_list)
        ...
        time_elapsed = (clusterEnd - clusterStart)
